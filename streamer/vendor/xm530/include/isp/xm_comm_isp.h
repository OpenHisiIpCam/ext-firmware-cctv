...
#define XM_ERR_ISP_NULL_PTR                                                    \
  XM_DEF_ERR(XM_ID_ISP, EN_ERR_LEVEL_ERROR, EN_ERR_NULL_PTR)
#define XM_ERR_ISP_ILLEGAL_PARAM                                               \
  XM_DEF_ERR(XM_ID_ISP, EN_ERR_LEVEL_ERROR, EN_ERR_ILLEGAL_PARAM)

#define XM_ERR_ISP_NOT_INIT                                                    \
  XM_DEF_ERR(XM_ID_ISP, EN_ERR_LEVEL_ERROR, ERR_ISP_NOT_INIT)
#define XM_ERR_ISP_TM_NOT_CFG                                                  \
  XM_DEF_ERR(XM_ID_ISP, EN_ERR_LEVEL_ERROR, ERR_ISP_TM_NOT_CFG)
#define XM_ERR_ISP_ATTR_NOT_CFG                                                \
  XM_DEF_ERR(XM_ID_ISP, EN_ERR_LEVEL_ERROR, ERR_ISP_ATTR_NOT_CFG)
#define XM_ERR_ISP_SNS_UNREGISTER                                              \
  XM_DEF_ERR(XM_ID_ISP, EN_ERR_LEVEL_ERROR, ERR_ISP_SNS_UNREGISTER)
#define XM_ERR_ISP_INVALID_ADDR                                                \
  XM_DEF_ERR(XM_ID_ISP, EN_ERR_LEVEL_ERROR, ERR_ISP_INVALID_ADDR)
#define XM_ERR_ISP_NOMEM                                                       \
  XM_DEF_ERR(XM_ID_ISP, EN_ERR_LEVEL_ERROR, ERR_ISP_NOMEM)
...
typedef struct xm_ISP_AE_ROUTE_NODE_S
{
  ...
  XM_U32 u32ApePercent; /* the percent of the iris's aperture, range is [0~100],
                           dc-iris not support in auto mode. */
} ISP_AE_ROUTE_NODE_S;
...
{
  ISP_AE_FRAME_END_UPDATE_0 =
    0x0, // isp update gain and exposure  in the  same frame
  ISP_AE_FRAME_END_UPDATE_1 = 0x1, // isp update exposure one frame before  gain
  ISP_AE_FRAME_END_UPDATE_2 =
    0x2, // isp update exposure one frame before  gain, isr config
...
{
  /* The exposure time is fixed at several values. The minimum exposure time is
   * 1/120 sec. (60 Hz) or 1/100 sec. (50 Hz). Since the exposure time is
   * synchronized with the fluorescent lighting, rolling bar noise can be
   * suppressed. In high-luminance environments, however, the exposure time is
   * fixed to the minimum exposure time, it may lead to over exposure
   */
  ISP_ANTIFLICKER_MODE_0 = 0x0,

  /* In this mode, the exposure time is not fixed in high-luminance environments
   * and the exposure time can be reduced  to the minimum . Use it when the
   * exposure is to be controlled in high-luminance environments. in low
   * lumiance enviroment, the effect is the same as the above one
   */
...
{
  XM_BOOL bEnable;
  XM_U8 u8Frequency; /*RW, Range: usually this value is 50 or 60  which is the
...
{
  XM_U16 u16BlackDelayFrame; // Need ToBrighter	delay = u16BlackDelayFrame x2
                             // (FrameNums)
  XM_U16 u16WhiteDelayFrame; // Need ToDarker		delay =
                             // u16WhiteDelayFrame x2 (FrameNums)
} ISP_AE_DELAY_S;
...
typedef struct xmISP_AE_ATTR_S
{
  /* base parameter */
  ISP_AE_RANGE_S
    stExpTimeRange; /*RW,  sensor exposure time (unit: us ), Range: [0x0,
                       0xFFFFFFFF], it's related to the specific sensor */
  ISP_AE_RANGE_S stAGainRange; /*RW,  sensor analog gain (unit: times, 10bit
                                  precision), Range : [0x400, 0xFFFFFFFF], it's
                                  related to the specific sensor */
  ISP_AE_RANGE_S stDGainRange; /*RW,  sensor digital gain (unit: times, 10bit
                                  precision), Range : [0x400, 0xFFFFFFFF], it's
                                  related to the specific sensor */
  ISP_AE_RANGE_S
    stISPDGainRange; /*RW,  ISP digital gain (unit: times, 10bit precision),
                        Range : [0x400, 0xFFFFFFFF], it's related to the ISP
                        digital gain range */
  ISP_AE_RANGE_S
    stSysGainRange; /*RW,  system gain (unit: times, 10bit precision), Range :
                       [0x400, 0xFFFFFFFF], it's related to the specific sensor
                       and ISP Dgain range */
  XM_U32
    u32GainThreshold; /*RW,  Gain threshold for slow shutter mode (unit: times,
                         10bit precision), Range : [0x400, 0xFFFFFFFF] */
  ...
  ********************************/
  ...
  XM_U16 u16AlgMode;
  ISP_AE_STRATEGY_E
    enAEStrategyMode;       /*RW, Support Highlight prior or Lowlight prior*/
...
{
  XM_S32 s32AGain;   /*RW,  sensor analog gain (unit: times), Range: [0x0,
                        0xFF],it's related to the specific sensor */
  XM_S32 s32DGain;   /*RW,  sensor digital gain (unit: times), Range: [0x0,
                        0xFF],it's related to the specific sensor */
  XM_U32 u32ExpLine; /*RW,  sensor exposure time (unit: line ), Range: [0x0,
                        0xFFFF],it's related to the specific sensor */
  XM_S32 s32IspGain;
  XM_S32 u32Exp;
...
typedef struct xm_ISP_ME_ATTR_EX_S
{
  ...
  XM_S32 s32DGain;    /*RW,  sensor digital gain (unit: times), Range: [0x0,
                         0xFF],it's related to the specific sensor */
  XM_U32 u32ISPDGain; /*RW,  sensor digital gain (unit: times), Range: [0x0,
                         0xFF],it's related to the specific isp   */
  XM_U32 u32ExpLine;  /*RW,  sensor exposure time (unit: line ), Range: [0x0,
                         0xFFFF],it's related to the specific sensor */
...
{
  XM_U16
    u16FocusMetrics; /*RO, The integrated and normalized measure of contrast*/
  XM_U16 u16ThresholdRead;  /*RO, The ISP recommend value of AF threshold*/
  XM_U16 u16ThresholdWrite; /*RW, The user defined value of AF threshold (or 0
                               to use threshold from previous frame)*/
  XM_U16 u16FocusIntensity; /*RO, The average brightness*/
  XM_U8 u8MetricsShift; /*RW, Metrics scaling factor:the bigger value for this
                           register means all zones metrics go higher,0x03 is
                           the default, Range: [0x0, 0xF]*/
  XM_U8 u8NpOffset;     /*RW, The AF noise profile offset, Range: [0x0, 0xFF]*/
  XM_U16 u16ZoneMetrics[AE_ZONE_ROW]
                       [AE_ZONE_COLUMN]; /*RO, The zoned measure of contrast*/
...
typedef struct xm_ISP_AWB_ATTR_S
{
  ...
  XM_U8 u8ZoneSel; /*RW,  A value of 0 or 0x3F means global AWB, A value between
                      0 and 0x3F means zoned AWB */
  XM_U8 u8HighColorTemp; /*RW, AWB max temperature in K/100, Range: [0x0, 0xFF],
                            Recommended: [u8LowColorTemp, 100] */
  XM_U8
    u8LowColorTemp; /*RW, AWB min temperature in K/100, Range: [0x0,
                       u8HighColorTemp), Recommended: [20, u8HighColorTemp] */
...
typedef struct xm_ISP_AWB_IN_OUT_ATTR_S
{
  ...
  XM_BOOL
    bOutdoorStatus;    /*in Auto mode, this is RO, in Manual mode, this is WO*/
  XM_U32 u32OutThresh; /*shutter time(in us) to judge indoor or outdoor */
  XM_U16 u16LowStart;  /*5000K is recommend. [u8LowStart, u8HighStart] is the
                          color temperature range of Day Light*/
  ...
  XM_BOOL
    bGreenEnhanceEn; /*If this is enabled, Green channel will be enhanced based
                        on the area of green plant, only take effect outdoor*/
...
typedef struct xm_ISP_AWB_CT_LIMIT_ATTR_S
{
  ...
  XM_U16
    u16HighRgLimit; /*RO, in Manual Mode, user should define the Max Rgain of
                       High Color Temperature, u16HighRgLimit > u16LowRgLimit*/
  XM_U16
    u16HighBgLimit; /*RO, in Manual Mode, user should define the Min Bgain of
                       High Color Temperature, u16HighBgLimit < u16LowBgLimit*/
  XM_U16 u16LowRgLimit; /*RO, in Manual Mode, user should define the Min Rgain
                           of Low Color Temperature*/
  XM_U16 u16LowBgLimit; /*RO, in Manual Mode, user should define the Max Bgain
                           of Low Color Temperature*/
...
typedef struct xm_ISP_ADV_AWB_ATTR_S
{
  ...
  XM_U8 u8Tolerance;  /*RW, Range:[0x0, 0xFF], AWB adjust tolerance,for outdoor,
                         this value should be small,recomend 4*/
  XM_U16 u16CurveLLimit; /*RW, Range:[0x0, 0x100],   Left limit of AWB Curve,
                            recomend for indoor 0xE0, outdoor 0xE0*/
  XM_U16 u16CurveRLimit; /*RW, Range:[0x100, 0xFFF], Right Limit of AWB
                            Curve,recomend for indoor 0x130, outdoor 0x120*/
...
{
  XM_U16 u16WhiteLevel; /*RW, Upper limit of valid data for white region, Range:
                           [0x0, 0xFFFF]*/
  XM_U16 u16BlackLevel; /*RW, Lower limit of valid data for white region, Range:
                           [0x0, u16WhiteLevel)*/
  XM_U16
    u16CbMax; /*RW, Maximum value of B/G for white region, Range: [0x0,0xFFF]*/
  XM_U16 u16CbMin; /*RW, Minimum value of B/G for white region, Range: [0x0,
                      u16CbMax)*/
  XM_U16
    u16CrMax; /*RW, Maximum value of R/G for white region, Range: [0x0, 0xFFF]*/
  XM_U16 u16CrMin; /*RW, Minimum value of R/G for white region, Range: [0x0,
                      u16CrMax)*/
  ...
  ISP_WB_ZONE_STA_INFO_S stZoneSta[AE_ZONE_ROW]
                                  [AE_ZONE_COLUMN]; /*RO, Zoned WB statistics*/
} ISP_WB_STA_INFO_S;
...
typedef struct xmISP_COLORMATRIX_AUTO_S
{
  ...
  XM_U16 u16MidColorTemp; /*RW,  the MidColorTemp should be at least 400 smaller
                             than HighColorTemp, Range: [u16LowColorTemp + 400,
                             u16HighColorTemp-400]*/
  XM_U16 au16MidCCM[12];  /*RW,  Range: [0x0,  0xFFFF]*/
  XM_U16
    u16LowColorTemp; /*RW,  the LowColorTemp should be at least 400 smaller than
                        HighColorTemp, Range: [0,  u16MidColorTemp-400]*/
...
{
  XM_U16 u16RedCastGain;   /*RW,  Range: [0x100, 0xFFFF], adjust the final red
                              channel  tone of the picture */
  XM_U16 u16GreenCastGain; /*RW,  Range: [0x100, 0xFFFF], adjust the final green
                              channel  tone of the picture*/
  XM_U16 u16BlueCastGain;  /*RW,  Range: [0x100, 0xFFFF], adjust the final blue
                              channel  tone of the picture*/
...
typedef struct xm_ISP_MI_ATTR_S
{
  ...
  XM_U16
    u16ApePercent; /* the percent of the iris's aperture, range is [0~100]. */
} ISP_MI_ATTR_S;
...
typedef struct xmISP_STDPC_ATTR_S
{
  ...
  ISP_STATIC_DP_TYPE_E
    enStaticDPType; /* Select static bright/dark defect-pixel calibration. */
  ...
  XM_U16 u16BadPixelCountMax;    /*RW, limit of max number of bad pixel,  Range:
                                    [0, 0x3FF] */
  XM_U16 u16BadPixelCount;       /*R	 DP count:[0, 0x3FF]*/
  XM_U16 u16BadPixelTriggerTime; /*RW, time limit for bad pixel trigger, in
                                    frame number ,Range: [0x0, 0x640]*/
  XM_U32
    u32BadPixelTable[STATIC_DP_COUNT_MAX]; /*RW, Range: [0x0, 0x3FFFFF],the
                                              first 11 bits represents the X
                                              coordinate of the defect pixel,
                                              the second 11 bits represent the Y
                                              coordinate of the defect pixel*/
} ISP_STDPC_ATTR_S;
...
typedef struct xm_ISP_SHADINGTAB_S
{
  ...
  XM_U16 u16ShadingTable_R[SHADING_TABLE_NODE_NUMBER_MAX]; /*RW, Range: [0x0,
                                                              0xFFFF]*/
  XM_U16 u16ShadingTable_G[SHADING_TABLE_NODE_NUMBER_MAX]; /*RW, Range: [0x0,
                                                              0xFFFF]*/
  XM_U16 u16ShadingTable_B[SHADING_TABLE_NODE_NUMBER_MAX]; /*RW, Range: [0x0,
                                                              0xFFFF]*/
  ...
  XM_U16 u16ShadingTableNodeNumber; /*RW, Range: [0x0,
                                       SHADING_TABLE_NODE_NUMBER_MAX]*/
} ISP_SHADINGTAB_S;
...
typedef struct xm_ISP_DENOISE_ATTR_S
{
  ...
  XM_U8 u8ThreshTarget; /*RW,  Noise reduction effect for high spacial
                           frequencies Range: [0x0, u8ThreshTarget]*/
  XM_U8 u8ThreshMax; /*RW,  Noise reduction effect for high spacial frequencies,
                        Range: [0x0, 0xFF] */
  XM_U8 u8SnrThresh[8]; /*RW,  Noise reduction target value array for  different
                           iso, Range: [0x0, 0xFF],*/
} ISP_DENOISE_ATTR_S;
...
typedef enum xm_ISP_GAMMA_CURVE_E
{
  ...
  ISP_GAMMA_CURVE_USER_DEFINE =
    0x6, /* user defined Gamma curve, Gamma Table must be correct */
...
typedef struct xm_ISP_CR_ATTR_S
{
  ...
  XM_U8 u8Sensitivity; /*Range: [0x0, 0xFF],this register is  not recommended
                          to change */
  XM_U16 u16Threshold; /*Range: [0x0, 0xFFFF],this register is  not recommended
                          to change */
  XM_U16 u16Slope; /*Range: [0x0, 0xFFFF],this register is  not recommended  to
                      change */
} ISP_CR_ATTR_S;
...
{
  XM_BOOL bEnable;
  XM_U8 u8Strength; /* Range: [0x0, 0xFF], the recommended range is [0x0, 0x95],
                     * the normal color will gradually be eroded when this
                     * register is larger than 0x95. */
...
typedef struct xmISP_AI_CALIBRATE_S
{
  ...
  XM_U16 u16StopValue;  /*RW, the initial stop value for AI calibraiton, Range:
                           [0x0,0x3E8]*/
  XM_U16 u16CloseValue; /*RW, the drive value to close Iris, Range: [0x0,0x3E8],
                           Recommended value: [700, 900]. A larger value means
                           faster.*/
  XM_U16 u16CalibrateTime; /*RW, frame numbers of AI calibraiton lasting time. >
                              600, [0x0, 0xFFF]*/
  XM_U8
    u8InertiaValue; /*RW, frame numbers of  AI moment of inertia, Range: [0x0,
                       0xFF],the recommended value is between[0x3, 0xa]*/
...
{
  XM_U8 u8Thresh; /* RW, Range: [0x0, 0xFF]. Noise reduction effect for high
                     spacial frequencies. */
...
typedef struct xmISP_2DNR_AUTO_ATTR_S
{
  XM_U8 u8GlobalSth; // [0, 0xFF]
  XM_U8
    au8Thresh[ISP_AUTO_STENGTH_NUM]; /* RW, Range: [0x0, 0xFF]. Noise reduction
                                        target value array for different ISO. */
...
typedef struct xmISP_WB_INFO_S
{
  ...
  XM_U16 u16ColorTemp;  /*RO, Detect color temperature, maybe out of color
                           cemeprature range*/
