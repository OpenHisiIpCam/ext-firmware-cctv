...
typedef struct xmIR_CTRL_ATTR_S
{
  ...
  unsigned char
    u8Mode; // 0: No PhotoR  1: Have PhotoR  2: No PhotoR_V2  3: Have PhotoR_V2
...
输入参数:	u8DnNow: 当前日夜模式(实际生效)
                                ...
                                pstIrDncAttr
                                        ...
                                        s32ColorDlt2:
>=该值进入彩色模式(Example: 50) s32BwExp:	>=该值进入黑白模式 输出参数:
无 返回参数:	-1:	出错 0:	彩色 1:	黑白
*********************************************************************************/
int
XM_MPI_IR_GetDNCStatus(unsigned char u8DnNow, IR_DNC_ATTR_S* pstIrDncAttr);
...
*********************************************************************************/
int
XM_MPI_IR_GetInfo(IR_DNC_INFO_S* pstDncInfo);
...
*********************************************************************************/
int
XM_MPI_IR_FunCallBack(IR_FUNC_S* pstRegister);
...
*********************************************************************************/
int
XM_MPI_IR_SetCtrlAttr(IR_CTRL_ATTR_S* pstCtrlAttr);
...
*********************************************************************************/
int
XM_MPI_IR_GetCtrlAttr(IR_CTRL_ATTR_S* pstCtrlAttr);
...
*********************************************************************************/
XM_S32
XM_MPI_IR_SwitchMode(XM_U8 u8Mode);
...
*************************************************************************/
int
XM_MPI_DBG_Task(unsigned char u8Num, unsigned int* pu32Data);
