 ...
 *  eg: 1/16, 2/16, 3/16 multiplies, enAccuType=AE_ACCURACY_LINEAR,
 * f32Accuracy=0.0625 eg: 1,2,4,8,16 multiplies, enAccuType=AE_ACCURACY_DB,
 * f32Accuracy=6
 */
...
typedef struct xm_AE_SENSOR_EXP_FUNC_S
{
  ...
  XM_VOID (*pfn_cmos_slow_framerate_set)
  (XM_U16 u16SlowFrameRate, AE_SENSOR_DEFAULT_S* pstAeSnsDft);
  ...
  XM_VOID (*pfn_cmos_shut_calc_table)
  (XM_S32 s32IntTime, AE_SENSOR_SHUTINFO_S* pstAeSnsShutInfo);
  XM_VOID (*pfn_cmos_gains_update2)
  (XM_S32 s32DGain, const AE_SENSOR_DEFAULT_S* pstAeSnsDft);

  XM_VOID (*pfn_cmos_again_calc_table)
  (XM_U32 u32InTimes, AE_SENSOR_GAININFO_S* pstAeSnsGainInfo);
  XM_VOID (*pfn_cmos_dgain_calc_table)
  (XM_U32 u32InTimes, AE_SENSOR_GAININFO_S* pstAeSnsGainInfo);
