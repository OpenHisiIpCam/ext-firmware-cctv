...
{
  XM_S32 (*pfn_ae_init)
  (ISP_DEV IspDev, XM_S32 s32Handle, const ISP_AE_PARAM_S* pstAeParam);
  XM_S32 (*pfn_ae_run)
  (ISP_DEV IspDev,
   XM_S32 s32Handle,
   ...
   XM_S32 s32Rsv);
  XM_S32 (*pfn_ae_ctrl)
  (ISP_DEV IspDev, XM_S32 s32Handle, XM_U32 u32Cmd, XM_VOID* pValue);
...
typedef struct xm_ISP_AWB_EXP_FUNC_S
{
  XM_S32 (*pfn_awb_init)
  (XM_S32 s32Handle,
   ISP_AWB_RESULT_BK_S* const pstAwbParam,
   XM_U32 Width,
   XM_U32 Height);
  XM_S32 (*pfn_awb_run)
  (XM_S32 s32Handle,
...
typedef struct xm_ISP_AF_STAT_S
{
  ...
  XM_U16 u16ThresholdWrite; /* The user defined value of AF threshold (or 0 to
                               use threshold from previous frame)*/
  XM_U16 u16FocusIntensity; /* The average brightness*/
  XM_U8 u8MetricsShift;     /* Metrics scaling factor:the bigger value for this
  ...
  XM_U16 au16ZoneMetrics[AE_ZONE_ROW]
                        [AE_ZONE_COLUMN]; /* The zoned measure of contrast*/
} ISP_AF_STAT_S;
