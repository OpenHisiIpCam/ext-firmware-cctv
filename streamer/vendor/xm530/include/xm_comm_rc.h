...
extern "C"
{
  ...
  typedef struct xmVENC_ATTR_H264_CBR_S
  {
    ...
    XM_U32 u32FluctuateLevel; /* level [0..5].scope of bitrate fluctuate. 1-5:
                                 10%-50%. 0: SDK optimized, recommended; */
  ...
  typedef struct xmVENC_PARAM_H264_CBR_S
  {
    ...
    XM_S32 s32IPQPDelta; /* the qp difference between the i frame and the before
                            gop avarage qp; == Qp(P) - Qp(I) */
  ...
  typedef struct xmVENC_PARAM_H264_VBR_S
  {
    XM_S32 s32IPQPDelta; /* the qp difference between the i frame and the before
                            gop avarage qp; == Qp(P) - Qp(I) */
    XM_S32 s32ChangePos; /* Indicates the ratio of the current bit rate to the
                            maximum bit rate when the QP value starts to be
                            adjusted */
    XM_U32 u32MinIprop;  /* the min ratio of i frame and p frame */
    XM_U32 u32MaxIprop;  /* the max ratio of i frame and p frame */
    XM_U32 u32MinIQP;    /* min qp for i frame */
  ...
  typedef struct xmVENC_PARAM_MJPEG_CBR_S
  {
    ...
    // XM_U32  u32RQRatio[RC_RQRATIO_SIZE];                              /* the
    // rate stabilization weight, 100-u32RQRatio[i] is the sequence quality
  ...
  {
    XM_S32 s32DeltaQfactor; /* Indicates the maximum change of Qfactor values of
                               frames when the picture quality changes */
    XM_S32
      s32ChangePos; /* Indicates the ratio of the current bit rate to the
                       maximum bit rate when the Qfactor value starts to be
                       adjusted */
  ...
  typedef struct xmVENC_PARAM_MPEG4_CBR_S
  {
    ...
    XM_U32 u32MaxQp;        /* the max QP value*/
    XM_U32 u32MinQp;        /* the min QP value */
    XM_U32 u32MaxPPDeltaQp; /* the max qp value difference between two
                               successive P frame */
    XM_U32 u32MaxIPDeltaQp; /* the max qp value difference between p frame and
                               the next i frame */
    XM_S32 s32IPQPDelta; /* the qp difference between the i frame and the before
                            gop avarage qp*/
  ...
  {
    // XM_U32 u32ThrdI[RC_TEXTURE_THR_SIZE];                     /* just useful
    // for h264/h265 and mpeg4 for now */ XM_U32 u32ThrdP[RC_TEXTURE_THR_SIZE];
  ...
  typedef struct xmVENC_PARAM_FRAMELOST_S
  {
    XM_BOOL bFrmLostOpen;    /* Indicates whether to discard frames to ensure
                                     stable bit rate when the instant bit rate is
                                exceeded */
  ...
  {
    VENC_SUPERFRM_MODE_E
      enSuperFrmMode; /* Indicates the mode of processing the super
                         frame,[SUPERFRM_NONE,SUPERFRM_DISCARD,SUPERFRM_REENCODE]*/
    XM_U32
      u32SuperIFrmBitsThr; /* Indicate the threshold of the super I frame
                                for enabling the super frame processing mode */
    XM_U32
      u32SuperPFrmBitsThr; /* Indicate the threshold of the super P frame */
    XM_U32
      u32SuperBFrmBitsThr; /* Indicate the threshold of the super B frame */
