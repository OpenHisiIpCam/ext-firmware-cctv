...
#define MAX_AUDIO_POINT_BYTES 4 /*max bytes of one sample point(now 32bit      \
                                   max)*/

#define MAX_VOICE_POINT_NUM 480 /*max sample per frame for voice encode */

#define MAX_AUDIO_POINT_NUM                                                    \
  2048 /*max sample per frame for all encoder(aacplus:2048)*/
...
#define MAX_AI_USRFRM_DEPTH 30 /*max depth of user frame buf */
  ...
  /*
  An example of the packing scheme for G726-32 codewords is as shown, and bit A3
  is the least significant bit of the first codeword: RTP G726-32: 0 1 0 1 2 3 4
  ...
  */
  ...
  {
    /* see DVI4 diiffers in three respects from the IMA ADPCM at
       RFC3551.txt 4.5.1 DVI4 */

    ADPCM_TYPE_DVI4 = 0, /* 32kbps ADPCM(DVI4) for RTP */
    ADPCM_TYPE_IMA,      /* 32kbps ADPCM(IMA),NOTICE:point num must be
                            161/241/321/481 */
    ADPCM_TYPE_ORG_DVI4,
    ADPCM_TYPE_BUTT,
  } ADPCM_TYPE_E;
...
#define AI_EXPAND 0x01
  ...
  typedef struct xmAIO_ATTR_S
  {
    ...
    XM_U32 u32EXFlag; /* expand 8bit to 16bit,use AI_EXPAND(only valid for AI
                         8bit) */
    XM_U32 u32FrmNum; /* frame num in buf[2,MAX_AUDIO_FRAME_NUM]*/
    XM_U32
      u32PtNumPerFrm; /* point num per frame (80/160/240/320/480/1024/2048)
                          (ADPCM IMA should add 1 point, AMR only support 160)*/
    XM_U32 u32ChnCnt; /* channle number on FS, valid value:2/4/8/16 */
    XM_U32 u32ClkSel; /* clock select, meaning is diffrent when master and slave
                         mode, if sio slave mode: 0: AD and DA clock of codec is
                         separate 1: AD and DA clock of codec is inseparate else
                         sio master mode: 0:ACKOUT clock is from sio 0 1:ACKOUT
                         clock is from sio 1/2
                                  */
  } AIO_ATTR_S;
  ...
  {
    AUDIO_AEC_MODE_E enAecMode; /* AEC mode, default is speaker,   0:receiver
  ...
  typedef struct xmAI_VQE_CONFIG_S
  {
    ...
    XM_S32 bAgcOpen;
    XM_S32
      s32SampleRate; /* Sample Rate£º8KHz/11.025K/12K/16KHz¡£default: 8KHz*/
    XM_S32 s32FrameSample; /* VQE frame length£º
                            sample rate 8KHz:  VQE frame length:
                            80/160/240/320/400/480£¬ default: 160; sample
                            rate 11.025K/12K/16KHz: VQE frame length:
                            160/320/480/960£¬ default: 160 */
...
/* invlalid device ID */
#define XM_ERR_AI_INVALID_DEVID                                                \
  XM_DEF_ERR(XM_ID_AI, EN_ERR_LEVEL_ERROR, EN_ERR_INVALID_DEVID)
/* invlalid channel ID */
#define XM_ERR_AI_INVALID_CHNID                                                \
  XM_DEF_ERR(XM_ID_AI, EN_ERR_LEVEL_ERROR, EN_ERR_INVALID_CHNID)
/* at lease one parameter is illagal ,eg, an illegal enumeration value  */
#define XM_ERR_AI_ILLEGAL_PARAM                                                \
  XM_DEF_ERR(XM_ID_AI, EN_ERR_LEVEL_ERROR, EN_ERR_ILLEGAL_PARAM)
/* using a NULL point */
#define XM_ERR_AI_NULL_PTR                                                     \
  XM_DEF_ERR(XM_ID_AI, EN_ERR_LEVEL_ERROR, EN_ERR_NULL_PTR)
/* try to enable or initialize system,device or channel, before configing
 * attribute */
#define XM_ERR_AI_NOT_CONFIG                                                   \
  XM_DEF_ERR(XM_ID_AI, EN_ERR_LEVEL_ERROR, EN_ERR_NOT_CONFIG)
/* operation is not supported by NOW */
#define XM_ERR_AI_NOT_SUPPORT                                                  \
  XM_DEF_ERR(XM_ID_AI, EN_ERR_LEVEL_ERROR, EN_ERR_NOT_SUPPORT)
/* operation is not permitted ,eg, try to change stati attribute */
#define XM_ERR_AI_NOT_PERM                                                     \
  XM_DEF_ERR(XM_ID_AI, EN_ERR_LEVEL_ERROR, EN_ERR_NOT_PERM)
/* the devide is not enabled  */
#define XM_ERR_AI_NOT_ENABLED                                                  \
  XM_DEF_ERR(XM_ID_AI, EN_ERR_LEVEL_ERROR, EN_ERR_UNEXIST)
...
/* no data in buffer */
#define XM_ERR_AI_BUF_EMPTY                                                    \
  XM_DEF_ERR(XM_ID_AI, EN_ERR_LEVEL_ERROR, EN_ERR_BUF_EMPTY)
/* no buffer for new data */
#define XM_ERR_AI_BUF_FULL                                                     \
  XM_DEF_ERR(XM_ID_AI, EN_ERR_LEVEL_ERROR, EN_ERR_BUF_FULL)
/* system is not ready,had not initialed or loaded*/
#define XM_ERR_AI_SYS_NOTREADY                                                 \
  XM_DEF_ERR(XM_ID_AI, EN_ERR_LEVEL_ERROR, EN_ERR_SYS_NOTREADY)

#define XM_ERR_AI_BUSY XM_DEF_ERR(XM_ID_AI, EN_ERR_LEVEL_ERROR, EN_ERR_BUSY)

/* invlalid device ID */
#define XM_ERR_AO_INVALID_DEVID                                                \
  XM_DEF_ERR(XM_ID_AO, EN_ERR_LEVEL_ERROR, EN_ERR_INVALID_DEVID)
/* invlalid channel ID */
#define XM_ERR_AO_INVALID_CHNID                                                \
  XM_DEF_ERR(XM_ID_AO, EN_ERR_LEVEL_ERROR, EN_ERR_INVALID_CHNID)
/* at lease one parameter is illagal ,eg, an illegal enumeration value  */
#define XM_ERR_AO_ILLEGAL_PARAM                                                \
  XM_DEF_ERR(XM_ID_AO, EN_ERR_LEVEL_ERROR, EN_ERR_ILLEGAL_PARAM)
/* using a NULL point */
#define XM_ERR_AO_NULL_PTR                                                     \
  XM_DEF_ERR(XM_ID_AO, EN_ERR_LEVEL_ERROR, EN_ERR_NULL_PTR)
/* try to enable or initialize system,device or channel, before configing
 * attribute */
#define XM_ERR_AO_NOT_CONFIG                                                   \
  XM_DEF_ERR(XM_ID_AO, EN_ERR_LEVEL_ERROR, EN_ERR_NOT_CONFIG)
/* operation is not supported by NOW */
#define XM_ERR_AO_NOT_SUPPORT                                                  \
  XM_DEF_ERR(XM_ID_AO, EN_ERR_LEVEL_ERROR, EN_ERR_NOT_SUPPORT)
/* operation is not permitted ,eg, try to change stati attribute */
#define XM_ERR_AO_NOT_PERM                                                     \
  XM_DEF_ERR(XM_ID_AO, EN_ERR_LEVEL_ERROR, EN_ERR_NOT_PERM)
/* the devide is not enabled  */
#define XM_ERR_AO_NOT_ENABLED                                                  \
  XM_DEF_ERR(XM_ID_AO, EN_ERR_LEVEL_ERROR, EN_ERR_UNEXIST)
/* failure caused by malloc memory */
...
/* no data in buffer */
#define XM_ERR_AO_BUF_EMPTY                                                    \
  XM_DEF_ERR(XM_ID_AO, EN_ERR_LEVEL_ERROR, EN_ERR_BUF_EMPTY)
/* no buffer for new data */
#define XM_ERR_AO_BUF_FULL                                                     \
  XM_DEF_ERR(XM_ID_AO, EN_ERR_LEVEL_ERROR, EN_ERR_BUF_FULL)
/* system is not ready,had not initialed or loaded*/
#define XM_ERR_AO_SYS_NOTREADY                                                 \
  XM_DEF_ERR(XM_ID_AO, EN_ERR_LEVEL_ERROR, EN_ERR_SYS_NOTREADY)
