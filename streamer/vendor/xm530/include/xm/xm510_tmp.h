...
#ifdef SOC_SYSTEM
int
GetConfigIntValue(char* pInFileName,
                  char* pInSectionName,
                  const char (*pInKeyName)[NCNUM],
                  char (*pOutKeyValue)[NCNUM]);
int
GetConfigStringValue(char* pInFileName,
                     char* pInSectionName,
                     const char (*pInKeyName)[NCNUM],
                     char (*pOutKeyValue)[NCNUM]);
int
CompareString(char* pInStr1, char* pInStr2);
int
GetKeyValue(FILE* fpConfig,
            const char (*pInKeyName)[NCNUM],
            char (*pOutKeyValue)[NCNUM]);
#endif
void
CameraConfigInit(UPSCALER_CFG* gstvstdvp);
XM_U8
IrledConfigInit(void);
int
string_get_value(const char* filename,
                 const char* str,
                 const char separate,
                 char* value,
                 char* raw);
