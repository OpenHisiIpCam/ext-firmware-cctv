...
typedef struct _dpc_info_
{
  ...
  XM_U16 u16BadPixelCountMax; /*RW, limit of max number of bad pixel,  Range:
                                 [0, 0x3FF] */
  XM_U16 u16BadPixelCount;    /*RW, limit of min number of bad pixel, Range: [0,
                                 0x3FF]*/
...
XM_S32
IspPthradMutex_lock(pthread_mutex_t* mutex);
XM_S32
IspPthradMutex_unLock(pthread_mutex_t* mutex);

XM_S32
Write_IspReg(XM_U32 u32Addr, XM_U32 u32Value);
XM_S32
Read_IspReg(XM_U32 u32Addr);
...
XM_S32
IspPthradMutex_lock(XM_S32* mutex);
XM_S32
IspPthradMutex_unLock(XM_S32* mutex);
...
XM_VOID*
IspMmap(XM_U32 u32PhyAddr, XM_U32 u32Size);
XM_S32
IspMunmap(XM_VOID* pVirAddr, XM_U32 u32Size);
XM_S32
IspDPCScan(XM_U16* pu16Addr, ISP_STDPC_ATTR_S* pstStDPAttr);
XM_VOID
IspAutoScaneTask();
XM_S32
IspAePhyStatWinInit(XM_U16 u16TotalH, XM_U16 u16TotalV);
XM_S32
ispMemnCpy(XM_S8* pSrc, XM_S8* pDst, XM_U32 u32Num);
XM_S32
ispMemnSet(XM_S8* pSrc, XM_S8 pDat, XM_U32 u32Num);

XM_S32
IspInit(XM_U32 u32SizeH, XM_U32 u32SizeV);

// Awb
XM_VOID
IspAwbInit(ISP_AWB_WDW_ATTR_S* pstWinAttr);

XM_S32
IspAwbRsltSet(ISP_AWB_RESULT_S* const pstAwbRslt);
