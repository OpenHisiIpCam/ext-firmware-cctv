...
0xFFFE] 0~0x7FFF			: 0x8000 ~ 0xFFFE	:  <0 note:
        u16ValH/u16ValV = 0xFFFF 时标准不写入
*************************************************************************/
XM_S32
VI_WinSet(XM_U8 u8Mode,
          XM_U8 u8Mirror,
          XM_U8 u8Flip,
          XM_U16 u16ValH,
          XM_U16 u16ValV);
...
int
XM_I2C_Ioctl(int cmd, I2C_DATA_S* pstI2CData);
int
XM_SPI_Ioctl(int cmd, XM_U32* pu32Data);

void
I2C_Write(XM_U32 addr, XM_U32 data);
void
PrintHex(unsigned char u8Num, XM_U64 u64Data);
void
PrintInt(unsigned char u8Num, int u32Data);
void
SysDelay_ms(unsigned int nms);

XM_S32
SysGetProductInfo_Ptr(XM_PRODUCT_INFO** pstProductInfo);
...
***********************************************************************/
XM_BOOL
SysReadFromFlash(XM_U8* pu8Data, XM_U32 u32Addr, XM_U32 u32Len);
/**********************************************************************
函数功能:	配置色同步
...
**********************************************************************/
XM_S32
XM_MPI_VENC_SetColor(XM_U8 u8BurstMode, XM_U8 u8ColorMode);
