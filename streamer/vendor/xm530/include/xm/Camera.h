...
#define NAME_LEN 16
  typedef struct ispconfig_json_s
  {
    ...
    *****************************************/
    ...
    /*****************************************
    u16LedIO: (0: default   常规模组)
            bit0~bit7: 	(红外灯GPIO，16进制表示)
            bit8~bit15:	(白光灯GPIO，16进制表示)
    *****************************************/
  ...
  */
  ...
  typedef struct CAMERA_COLOR
  {
    ...
    unsigned char
      Gain; ///< 增益，取值0-100。bit7置位表示自动增益，其他位被忽略。
    unsigned char
      WhiteBalance; ///< 自动白电平控制，bit7置位表示开启自动控制.0x0,0x1,0x2分别代表低,中,高等级
    unsigned short
      Acutance; ///< 锐度，取值0-15, 第8位表示水平锐度，高8为表示垂直锐度。
  ...
  *************************************************************************/
  ...
  输入参数:	level:	0		自动曝光
                                                  1~9
  手动曝光(固定几档) value1:	曝光下限(min,us) value2:	曝光上限(max,us)
  note:
          自动曝光(上下限无效)  		level :0
          手动曝光(固定几档)			level :1~9
  *************************************************************************/
  int camera_get_exposure_level(int* pLevel, XM_U32* pValue1, XM_U32* pValue2);
  int camera_set_exposure_level(int level,
                                unsigned int value1,
                                unsigned int value2);
  ...
  输入参数:	s32GainMax:	最大增益	( 0~100	def: 50)
                                  s32AutoEn:	AutoGain 使能	(0~1
  def: 1) 输出参数:	无 返回参数:	0 	成功 -1	出错 Note:
  Lycai
  *******************************************************************/
  ...
  函数功能:	获取当前背光补偿参数
  输入参数:	无
  输出参数:	*pu32Enable 当前使能开关(0/1)
                          *pu32Level 当前强度(0~100)
  返回参数:	0:	成功
                                  -1:	失败
  *******************************************************************/
  ...
  *******************************************************************/
  ...
  int camera_set_ae_sensitivity(
    int ae_sensitivity); /// ae灵敏度配置 1-10，默认为5
  ...
  *******************************************************************/
  ...
  // int camera_set_RGBGain(int channel, int gainval);//设置RGB增益1 RED通道  2
  // GREEN通道  3BLUE通道
  int camera_get_Infrared(
    void); //获取设备红外状态 1表示红外打开 0表示关闭 2表示芯片不支持
  ...
  *******************************************************************/
  ...
  int Camera_Get_DebugFile(char* fliename,
                           unsigned int choice,
                           unsigned int depth);
  ...
  *************************************************************************/
  ...
  2:SubStream2 pstFormat: encode paramer 输出参数:	none 返回参数:	0:
  成功 -1:	出错
  *************************************************************************/
  int camera_set_format(int chn,
  ...
  *************************************************************************/
  ...
  输入参数:	ps32Mode:	0xAB(	A:	0: IR   1: WL   --- 仅读取时有效
                                                                  B:	0:Auto
  1:Manual  2:Intelligence) ps32State: 	0:Close 1:Open 输出参数:	无
  返回参数:	0:	成功
                                  -1:	出错
  *************************************************************************/
  ...
  *************************************************************************/
  ...
  /*****************************************************************************
  函数功能:	外部调试选择gamma
  输入参数:    	u32Gamma: [High Byte->Low Byte] = [u8IdxWgt(Idx2)  u8Idx2
  u8Idx1] u8Idx1:	0: 		disable gamma  (线性) 1~15:
  LinearGamma 128~255:	WdrGamma u8Idx2:	0: 		disable gamma
  (线性) 0~15:	LinearGamma 128~255:	WdrGamma u8IdxWgt: (0~255) u8Idx1
  Weight: 255-u8IdxWgt u8Idx2 Weight: u8IdxWgt 返回参数:    0: 成功 -1: 失败
  *****************************************************************************/
  ...
  *************************************************************************/
  ...
  typedef struct xm_isp_get_ai_result_s
  {
    ...
    ***/
    ...
    unsigned int u32AlarmFlag; /*触发报警(人形/车牌)1：报警(检测到目标)
                                  0：不报警(未检测到目标)*/
  ...
  typedef struct xm_ISP_GET_AI_FUNC_S
  {
    ...
    (ISP_GET_AI_RESULT_S* pstAiResult); // 返回值：0:成功      	-1:失败
  } ISP_GET_AI_FUNC_S;
  ...
  typedef struct xm_CAMERACFG_TO_ISP
  {
    ...
    u32IRLed:
            bit31:
                    ...
                    1	use
            bit23:	控制白光灯的电平
            bit16~bit22:	控制白光灯的GPIO	(0x00: choice by
    source,100:表示GPIO00) bit15: 	控制红外灯的电平 bit8~bit14:
    控制红外灯的GPIO (0x00: choice by source,100:表示GPIO00)
    ...
    ****************************/
    ...
    unsigned int
      au32RsltCh[4][4]; // (Ch0_Width Ch0_Height Ch1_Width Ch1_Height) * 4
    ...
    u32Infrared:
            ...
            bit8~bit15: InfraredGPIO (0x00: choice by source,100:表示GPIO00)
            bit0: InfraredSwap
    ****************************/
    ...
    u32IrCut:
            bit31:
                    0	not use
                    1	use
            bit8~bit15: IrcutGPIO (0x00: choice by source,100:表示GPIO00)
            bit0: IRCUT Swap
    ****************************/
