...
#include "xm_common.h"
  ...
  ** ADVICE:
  ** 1. Bester to call XM_MPI_SYS_GetCurPts on the host board to get the
  *u64Base.
  ** 2. When the linux start up, call XM_MPI_SYS_InitPtsBase to set the init
  *pts.
  ...
  */
  ...
  XM_S32 XM_MPI_SYS_MmzAlloc(XM_U32* pu32PhyAddr,
                             XM_VOID** ppVirtAddr,
                             const XM_CHAR* strMmb,
                             const XM_CHAR* strZone,
                             XM_U32 u32Len);

  XM_S32 XM_MPI_SYS_MmzAlloc_Cached(XM_U32* pu32PhyAddr,
                                    void** ppVitAddr,
                                    const XM_CHAR* pstrMmb,
                                    const XM_CHAR* pstrZone,
                                    XM_U32 u32Len);

  /* free mmz memory in user context                                          */
  XM_S32 XM_MPI_SYS_MmzFree(XM_U32 u32PhyAddr, XM_VOID* pVirtAddr);
  XM_S32 XM_MPI_SYS_MmzFlushCache(XM_U32 u32PhyAddr,
                                  XM_VOID* pVirAddr,
                                  XM_U32 u32Size);
